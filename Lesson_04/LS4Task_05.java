...
public class LS4Task_05 {
    ...
    public static String infixToPostfix(String infix) {
        ...
        for (char c : infix.toCharArray()) {
            // если встреченный символ является операндом добавляем его в выходные данные,
            // т.е. постфикс
            if (Character.isLetterOrDigit(c)) {
                ...
                stk.push(c);
                // если встреченный символ равен ')', извлекаем стек(stk) до тех пор, пока не
                // будет обнаружен '('
            } else if (c == ')') {
                while (!stk.isEmpty()
                        && stk.peek() != '(') {
                ...
                // если встреченный символ не является круглой скобкой или операндом, то
                // проверяем

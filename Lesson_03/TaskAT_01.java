...
class MergeSort {
    ...
    // собственно слияние (merge) самая сложная часть алгоритма, поэтому для него
    // отдельный метод
    static void merge(int nums[], int left, int m, int right) {
        ...
        // оставил для наглядности то, что с помощью компилятора я заменил на
        // System.arraycopy
        // for (int i = 0; i < n1; i++)
        ...
        while (i < n1 && j < n2) { // проходим одновремеенно левую и правильную половинки
            // т.к. есть копии, то можно уже заполнять отрезок исходного массива локально
